...
@Component({
  selector: 'app-tartas-tradicionales',
  templateUrl: './tartas-tradicionales.component.html',
  styleUrls: ['./tartas-tradicionales.component.css']
})
export class TartasTradicionalesComponent implements OnInit {
  @Input() producto: any
  ...
  tartasTradicionales: Producto[] = [];

  constructor(private authService: AuthService, private productService: ProductsService) { }

  ngOnInit(): void {
    this.getTartasTradicionales();
  }

  getTartasTradicionales(): void {
    this.productService.getTartasTradicionales().subscribe(products => {
      this.tartasTradicionales = products;
    });
  }
  ...
  addToFavorites(id: string) {
    ...
    if (existingIndex === -1) {
      ...
      favoritesFromLocal.push(this.tartasTradicionales.find(producto => producto._id === id));
    } else {
  ...
  onProductoBorrado(id: string) {
    this.tartasTradicionales = this.tartasTradicionales.filter(producto => producto._id !== id);
  }
}
